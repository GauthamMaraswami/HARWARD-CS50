...
{
    // TODO
    char* html_indx = "index.html";
    char* php_indx = "index.php";
    int length_html = (strlen(path) + strlen(html_indx) + 1);
    int length_php = (strlen(path) + strlen(php_indx) + 1);
    char* temp_path1 = malloc(length_html + 1);
    temp_path1 = strcpy(temp_path1, path);
    char* temp_path2 = malloc(length_php + 1);
    temp_path2 = strcpy(temp_path2, path);
    temp_path1 = strcat(temp_path1, html_indx);
    temp_path2 = strcat(temp_path2, php_indx);
    if (access(temp_path1, F_OK) == 0)
    {
        free(temp_path2);
        return temp_path1;
    }
    else if(access(temp_path2, F_OK) == 0)
    {
        free(temp_path1);
        return temp_path2;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    for (int c = fgetc(file); c != EOF; c = fgetc(file))
    {
        ...
        counter++;
        buffer = (char*) realloc(buffer, sizeof(BYTE) * (counter + 1));
    }
...
const char* lookup(const char* path)
{
    // create data type with an extension and corresponding response
    typedef struct MIME
    {
        char* extension;
    ...
    MIME;
    // initialize all possible MIME types
    MIME var[8];
    var[0].extension = ".css";
    var[0].MIME_type = "text/css";
    var[1].extension = ".html";
    var[1].MIME_type = "text/html";
    var[2].extension = ".gif";
    var[2].MIME_type = "image/gif";
    var[3].extension = ".ico";
    var[3].MIME_type = "image/x-icon";
    var[4].extension = ".jpg";
    var[4].MIME_type = "image/jpeg";
    var[5].extension = ".js";
    var[5].MIME_type = "text/javascript";
    var[6].extension = ".php";
    var[6].MIME_type = "text/x-php";
    var[7].extension = ".png";
    var[7].MIME_type = "image/png";
   
    // search for the dot
    const char dot = '.';
    char *ext;
    ext = strrchr(path, dot);
    // return corresponding type
    for (int i = 0; i < 8; i++)
    {
        if (strcasecmp(ext, var[i].extension) == 0)
        {
            return var[i].MIME_type;
            break;
        }
    }
    return NULL;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    int counter = 0;
    // check for the consistency of a request-line
    ...
    // retrieve request target and http version
    char *line1;
    line1 = strchr(line, '/');
    if (strncmp(line1, "/1.", 3) == 0)
    {
        error(501);
        return false;
    }
    ...
    http = strchr(line1, ' ');
    // check HTTP vesion
    if (http == NULL || strcasecmp(http, " HTTP/1.1\r\n") != 0)
    {
        error(505);
        return false;
    }
    // extract abs_path and store it in the temporal variable
    int length1 = strlen(line1);
    char* temp_abs = malloc(length1);
    // create temp value for the query
    char* temp_query = NULL;
    //dispose of garbage values
    ...
    // check for query
    if (strchr(temp_abs, '?') != NULL)
    {
        temp_query = strchr(temp_abs, '?');
        int length_query = strlen(temp_query);
        if (length_query == 1)
        {
            temp_query = "";
            return true;
        ...
        {
            // copy query to a temporal vatiable
            char* temp = malloc(length_query + 1);
            temp = strcpy(temp, temp_query);
            // get rid of query in temp_abs
            if (strstr(temp_abs, temp_query) != NULL)
            {
                *temp_query = '\0';
            }
            // get rid of "?"
            memmove(&temp[0], &temp[1], strlen(temp));
            // copy temporal values into correct place
            abs_path = strcpy(abs_path, temp_abs);
            query = strcpy(query, temp);
